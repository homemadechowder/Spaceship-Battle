...
//*****************************************************************************
#define DC_BASE GPIOA3_BASE
#define DC_CONTROL  0x40
#define RESET_BASE GPIOA3_BASE
#define RESET_CONTROL 0x10
#define OLEDCS_BASE GPIOA0_BASE
#define OLEDCS_CONTROL 0x1
...
*  SPI.
*/
    // write DC_control pin to low
    unsigned long trash;
    MAP_SPICSEnable(GSPI_BASE);
    GPIOPinWrite(DC_BASE, DC_CONTROL, 0);
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_CONTROL, 0);

    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &trash);

    GPIOPinWrite(DC_BASE, DC_CONTROL, DC_CONTROL);
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_CONTROL, OLEDCS_CONTROL);
...
*/
  ...
  GPIOPinWrite(GPIOA0_BASE, RESET_CONTROL, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA0_BASE, RESET_CONTROL, 0x40);	// RESET = RESET_HIGH
